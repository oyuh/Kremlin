...
public final class Combat implements Listener, CommandExecutor {
    ...
    void enable() {
        ...
        TeamHook teamHook = new TeamHook(plugin);
        teamHook.onTeamHomeTeleport(plugin, this::teamHomeTeleport);
        barMenu = new BarMenu(this);
        getServer().getPluginManager().registerEvents(this, plugin);
        getServer().getPluginManager().registerEvents(barMenu, plugin);
        tpa = new Tpa(this);
        tpa.load();
        ...
        players = new PlayerList(this, teamHook);
        getServer().getPluginManager().registerEvents(players, plugin);
        getServer().getPluginManager().registerEvents(new TabComplete(this), plugin);
        getLogger().info("Took over the tpa commands (Essentials' own are intercepted, /essentials:tpa included).");
    ...
    static String rootAndSub(String message) {
    ...
    Component msg(String key, TagResolver... resolvers) {
        ...
        if (raw == null || raw.isEmpty()) {
            // Should be unreachable now the jar's config is layered in as defaults, but an
            // empty message here once meant "silently cancel the command with no reason given".
            getLogger().warning("Message '" + key + "' is empty -- check plugins/Kremlin/config.yml");
     ...
     */
    void startWarmup(Player mover, Location destination, UUID partnerId, long warmupMillis) {
    ...
    /**
     * SimpleTeams' /team home, wearing our rules: combat refuses it outright, fleeing lengthens
     * it, and everyone else gets the same base warmup /hq has. We finish the teleport ourselves
     * rather than handing back to SimpleTeams, so its event never fires a second time.
     */
    ...
    Component denyTeleport(Player p) {
        return denyCombat(p);
    }
